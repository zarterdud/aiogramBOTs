...
async def start_work(message: types.Message):
    ...
    await message.answer("Сначала отправь нам фотографию!", reply_markup=cansle_kb())
...
async def load_photo(message: types.Message, state: FSMContext):
    ...
    await message.answer("А теперь отправь нам описание!")
...
async def load_photo(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        await bot.send_photo(
            chat_id=message.from_user.id, photo=data["photo"], caption=data["desc"]
        )
    await state.finish()
